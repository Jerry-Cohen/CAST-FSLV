...
P_ANALYTICAL = Path(r"4_Analytical Method for FSLV Prediction - CAST PTAP COMPLETE.csv")
...
def linear_fit_with_ci(x, y, x_pred):
    ...
    y_pred = a + b * x_pred
    ci_half = tcrit * np.sqrt(s2 * (1.0/n + (x_pred - x_bar)**2 / Sxx))
    return (a, b, s), (y_pred, y_pred - ci_half, y_pred + ci_half)

def ordinal_ticks(ax, vals):
    ticks = np.linspace(vals.min(), vals.max(), 5)
    labs = [pd.Timestamp.fromordinal(int(v)).strftime("%Y-%m-%d") for v in ticks]
    ax.set_xticks(ticks)
...
analytical = pd.read_csv(P_ANALYTICAL, dtype=str, encoding="utf-8-sig")
...
ANA_ID_COL  = analytical.columns[0]
ANA_PTA_COL = "First Estimated PTA Treatment Date"
ana = analytical[[ANA_ID_COL, ANA_PTA_COL]].rename(columns={ANA_ID_COL: "study_number", ANA_PTA_COL: "Estimated PTA"}).copy()
ana["study_number"] = ana["study_number"].map(norm_id)
ana["Estimated PTA"] = parse_dates_safely(ana["Estimated PTA"])
ana = ana[ana["study_number"].isin(master)].drop_duplicates("study_number")
...
merged = (
    pd.DataFrame({"study_number": master})
      ...
      .merge(ana, on="study_number", how="left")
...
for _, r in merged.iterrows():
    est = r.get("Estimated PTA")
    if pd.notna(est) and pd.notna(r.get("Calculated PTA (Planned)")):
        rows.append({
            "study_number": r["study_number"], "type": "Planned",
            "Calculated_PTA": r["Calculated PTA (Planned)"], "Estimated_PTA": est,
            "Delta_days": (est - r["Calculated PTA (Planned)"]).days
        })
    if pd.notna(est) and pd.notna(r.get("Calculated PTA (Actual)")):
        rows.append({
            "study_number": r["study_number"], "type": "Actual",
            "Calculated_PTA": r["Calculated PTA (Actual)"], "Estimated_PTA": est,
            "Delta_days": (est - r["Calculated PTA (Actual)"]).days
        })
tidy = pd.DataFrame(rows)


print("Merged rows (should be 58):", len(merged))
print("Estimated PTA non-null:", merged["Estimated PTA"].notna().sum())
...
def scatter_fit(df, title):
    if df.empty:
        print(f"{title}: no data")
        return
    x = df["Calculated_PTA"].map(lambda d: d.toordinal()).values.astype(float)
    y = df["Estimated_PTA"].map(lambda d: d.toordinal()).values.astype(float)
    if len(x) < 3:
        print(f"{title}: not enough points for fit")
        return
    x_pred = np.linspace(x.min(), x.max(), 200)
    fit, ci = linear_fit_with_ci(x, y, x_pred)
    if fit is None:
        print(f"{title}: fit failed")
        return
    (a,b,s), (y_pred, lo, hi) = fit, ci
    fig, ax = plt.subplots()
    ax.scatter(x, y, label="studies")
    ax.plot(x_pred, y_pred, label=f"fit (RMSE≈{s:.0f} days)")
    ax.fill_between(x_pred, lo, hi, alpha=0.2, label="95% CI (mean)")
    mn, mx = min(x.min(), y.min()), max(x.max(), y.max())
    ax.plot([mn, mx], [mn, mx], linestyle="--", label="y = x")
    ordinal_ticks(ax, np.array([mn, mx]))
    yticks = np.linspace(mn, mx, 5)
    ax.set_yticks(yticks); ax.set_yticklabels([pd.Timestamp.fromordinal(int(v)).strftime("%Y-%m-%d") for v in yticks])
    ax.set_title(title)
...
def residual_hist(df, title):
    ...
    if len(vals) == 0:
        print(f"{title}: no residuals")
        return
    fig, ax = plt.subplots()
    ax.hist(vals, bins=min(30, max(6, int(len(vals)**0.5))))
    ax.set_title(title + " — Residuals (Estimated − Calculated) [days]")
...
def bland_altman(df, title):
    ...
    est  = df["Estimated_PTA"].astype("datetime64[ns]")
    x = ((calc + (est - calc)/2)).map(lambda d: d.toordinal()).values.astype(float)   # mean date
    y = (est - calc).dt.days.values.astype(float)                                     # Δ days
    if len(x) == 0:
        print(f"{title}: no points")
        return
    ...
    ordinal_ticks(ax, np.array([x.min(), x.max()]))
    ax.set_title(title + " — Bland–Altman")
    ax.set_xlabel("Mean date (Calculated & Estimated)")
    ax.set_ylabel("Δ days (Estimated − Calculated)")
    ax.legend()
    plt.tight_layout()
    plt.show()
...
scatter_fit(tidy_planned, "Planned: Estimated vs Calculated PTA")
scatter_fit(tidy_actual,  "Actual: Estimated vs Calculated PTA")
